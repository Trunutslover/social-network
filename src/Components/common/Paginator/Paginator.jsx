...
const Paginator = (props) => {
    ...
    return (
        <div className={classes.pagination}>
            {currentPortion > 0 ? <button className={classes.prevButton} onClick={() => setCurrentPortion(currentPortion - 1)}>Prev</button> : null}
            {buttonsListFiltered}
            {currentPortion < Math.ceil(pagesCount / portionSize) ?
                <button className={classes.nextButton} onClick={() => setCurrentPortion(currentPortion + 1)}>Next</button> : null}
        </div>
    )
};
